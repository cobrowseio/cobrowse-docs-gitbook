...
1. **Redis** - we require access to a redis cluster. For high availability in self-hosted\
   environments, we recommend running Redis in cluster mode. Bitnami provide an easy\
   to use [Helm chart](https://github.com/bitnami/charts/tree/master/bitnami/redis-cluster). If you are using a managed Redis service (such as AWS ElastiCache, Google Cloud Memorystore, or Azure Cache for Redis), cluster mode is not required as these services handle high availability natively when deployed to multiple regions.
